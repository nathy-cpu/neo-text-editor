...
int Terminal_EnableRawMode(Terminal *terminal) {
    ...
    if (terminal->rawModeEnabled) return 0; // Already enabled
    if (!isatty(STDIN_FILENO)) return -1;
    if (tcgetattr(STDIN_FILENO, &terminal->originalTermios) == -1) return -1;
    ...
    if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &rawTermios) < 0) return -1;
...
void Terminal_HandleSignal(Terminal *terminal, int signalNumber) {
    if (terminal) Terminal_Restore(terminal);
