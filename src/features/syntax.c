...
char* Cppkeywords[] = {
    "class", "delete", "new", "namespace", "try", "catch", "throw", "public", "private", "protected", "virtual", "template", "typename", NULL
};

char* Cpptypes[] = {
    "bool", "char8_t", "char16_t", "char32_t", "wchar_t", NULL
};
...
char* GetSyntaxColor(int highlight)
{
    switch (highlight)
    {
        ...
        case HIGHLIGHT_KEYWORD: return "33";
        case HIGHLIGHT_TYPE: return "32";
        default: return "39";
    }
}
            ...
            if (prev_sep) {
                ...
                if (syntax->keywords) {
                    for (int j = 0; syntax->keywords[j]; j++) {
                        ...
                        if (!strncmp(&text[i], syntax->keywords[j], klen) && (i + klen == length || is_separator(text[i + klen]))) {
                ...
                if (!matched && syntax->types) {
                    for (int j = 0; syntax->types[j]; j++) {
                        ...
                        if (!strncmp(&text[i], syntax->types[j], klen) && (i + klen == length || is_separator(text[i + klen]))) {
...
void Tab_SetSyntaxHighlight(Tab* tab)
{
    ...
    for (unsigned int j = 0; HLDB[j].fileType != NULL; j++)
    {
        ...
        for (unsigned int i = 0; syn->fileMatch[i] != NULL; i++)
        {
            ...
            if ((isExt && ext && !strcmp(ext, syn->fileMatch[i])) || (!isExt && strstr(tab->filename, syn->fileMatch[i])))
            {
                tab->syntax = syn;
